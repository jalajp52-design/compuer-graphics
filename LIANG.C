...
void main() {
    ...
    // Removed the immediate line() call from here so it doesn't 
    // show up if the line is outside.
    ...
    setcolor(WHITE);
    rectangle(xmin, ymin, xmax, ymax); // Redraw window
    ...
    if (t1 < t2) {
        ...
        // No line() call here, so nothing is drawn
